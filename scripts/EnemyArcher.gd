...
@export var max_health: int           = 14   # doubled from 7
...
func _ready() -> void:
	...
	if _lbl:
		...
		if _sprite.setup(_lbl, "archer"):
			...
			for mk in fm:
				set_meta(mk, fm[mk])
			AsciiSprites.apply_hitbox(self, "archer")
		else:
			_sprite = null
...
func take_damage(amount: int, _source: Node = null) -> void:
	...
	if health <= 0:
		...
		GameState.kills += 1
		GameState.add_xp(5)
		# One-bag-per-enemy invariant — gold pickup always, then either the
		# champion's guaranteed bag or the regular probabilistic bag (never
		# both). Fixes "champions drop 3 bags at high difficulty" piles.
		_drop_gold_pickup()
		if is_champion:
			_drop_champion_loot()
		else:
