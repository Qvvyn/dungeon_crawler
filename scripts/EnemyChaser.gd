...
var _flank_side: float     = 1.0  # +1 or -1, randomized on spawn
var _sep_force: Vector2    = Vector2.ZERO
...
func _chase(delta: float) -> void:
	...
	if _sep_timer <= 0.0:
		...
		for other in get_tree().get_nodes_in_group("enemy"):
			...
			var d := diff.length()
			if d < 34.0 and d > 0.0:
				_sep_force += diff.normalized() * (34.0 - d) * 2.5

	var base_vel := move_dir * speed * _speed_multiplier * slow_mult
	velocity = (base_vel + _sep_force).limit_length(speed * _speed_multiplier * slow_mult * 1.3)
