...
static func spawn_death_pop(pos: Vector2, scene_root: Node, _color: Color = Color(1.0, 0.4, 0.4)) -> void:
	if not is_instance_valid(scene_root) or not scene_root.is_inside_tree():
		return
	# Death marker is "x_x" in black with a bright outline so it reads as a
	# universal "dead" face regardless of who killed it. The wand color
	# argument is ignored on death — the marker is meant to read the same
	# for every death.
	var death_col := Color(0.0, 0.0, 0.0)
	var lbl := Label.new()
	lbl.text = "x_x"
	lbl.add_theme_font_override("font", _font())
	lbl.add_theme_font_size_override("font_size", 22)
	lbl.add_theme_color_override("font_color", death_col)
	lbl.add_theme_color_override("font_outline_color", Color(1.0, 1.0, 1.0))
	lbl.add_theme_constant_override("outline_size", 3)
	lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	lbl.vertical_alignment   = VERTICAL_ALIGNMENT_CENTER
	lbl.position = pos - Vector2(24.0, 14.0)
	lbl.size     = Vector2(48.0, 28.0)
	lbl.z_index  = 4
	lbl.mouse_filter = Control.MOUSE_FILTER_IGNORE
	lbl.pivot_offset = Vector2(24.0, 14.0)
	...
	if GameState.active_rig != null and is_instance_valid(GameState.active_rig) \
		...
		GameState.active_rig.spawn_burst_2d(pos, "x_x", death_col, 1, 0.0, 0.45,
